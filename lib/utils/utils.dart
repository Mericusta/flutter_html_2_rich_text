...
// NOTE: 处理 p 标签的 text-align 时改变其 container 的 alignment，但是无法处理 justify
...
Color string2Color(String colorString) {
  ...
  // NOTE: 在 rgb 前检查是否是 rgba 形式的，可以免去使用正则表达式匹配
...
TextStyle convertTextStyle(Map<String, String> styleMap) {
  ...
  return (effectiveStyleMap == null || effectiveStyleMap.isEmpty) ? new TextStyle() : new TextStyle(
    ...
    background    : effectiveStyleMap.containsKey('background-color') ? (() => new Paint()..color = string2Color(effectiveStyleMap['background-color']))() : null,
